...
class BookingSystemServiceProviderImpl(IBookingSystemServiceProvider):
    ...
    def book_tickets(self, num_tickets):
        print("Enter the details of customer for booking:")
        ...
        print("select one of the events from below:")
        cursor.execute("select event_name from events")
        events = cursor.fetchall()
        for event in events:
            print(event[0])
        name_of_event = input("What is your event")
        ...
        if booking_id:
            b_id = booking_id[0]
        print("Congrats.Your booking is confirmed.Your booking id is ", b_id)
